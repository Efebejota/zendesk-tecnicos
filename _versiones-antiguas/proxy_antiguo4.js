...
async function loadAll(force) {
  ...
  console.log('Tickets (todos los estados)...');
  // El endpoint /tickets.json devuelve open/pending/hold/solved pero NO closed por defecto
  // Para obtener closed hay que solicitarlo explícitamente usando views o la API de tickets por estado
  // La forma más completa es cargar cada estado por separado
  const statuses = ['open', 'pending', 'hold', 'solved', 'closed'];
  let allTickets = [];
  for (const status of statuses) {
    console.log(`  Cargando estado: ${status}...`);
    try {
      const statusTickets = await fetchAll(BASE + `/tickets.json?status=${status}&per_page=100&sort_by=created_at&sort_order=asc`);
      console.log(`  -> ${statusTickets.length} tickets ${status}`);
      allTickets = allTickets.concat(statusTickets);
    } catch(e) {
      console.log(`  Error cargando ${status}: ${e.message}`);
    }
  }
  // Deduplicar por id
  const seen = new Set();
  allTickets = allTickets.filter(t => { if(seen.has(t.id)) return false; seen.add(t.id); return true; });
  console.log('Total sin duplicados: ' + allTickets.length);
